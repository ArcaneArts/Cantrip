import { persistModelBehaviorObservation } from "./model-behavior-writes.js";
...
export interface ModelBehaviorObservationInput {
  nativeAttribution?: import("@cantrip/protocol").NativeBehaviorAttribution;
  sourceKey: string;
  projectId: string | null;
  chatId: string | null;
  modelRouteId: string;
...
export class TelemetryRepository {
  ...
  async recordModelBehaviorObservation(
    ...
    return persistModelBehaviorObservation(this.database, ownerId, input);
  }
  ...
  async getProviderTelemetryAnalytics(
    ...
    return {
      ...
      behavior: {
        ...
        reasoningEfforts: behaviorBreakdown(behaviorRows, (row) =>
          row.nativeAttribution &&
          row.signalAvailability.nativeReasoningKnown !== true
            ? "unattributed"
            : (row.reasoningEffort ?? "provider-default"),
        ),
      },
      changePoints,
    };
  ...
  async exportProviderTelemetry(
    ...
    return {
      ...
      modelBehavior: behaviorRows.map((row) => ({
        ...(row.nativeAttribution
          ? { nativeAttribution: row.nativeAttribution }
          : {}),
        id: row.id,
        sourceKey: row.sourceKey,
        projectId: row.projectId,
