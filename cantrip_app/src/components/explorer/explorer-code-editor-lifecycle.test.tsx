...
const desktopCode = vi.hoisted(() => ({
  ...
  shouldUseLegacyProtectedCodeAttachmentFallback: vi.fn(),
...
vi.mock("@/components/code/code-view", () => ({
  ...
  codeWorkbenchSurfaceBackground: () => "transparent",
...
beforeEach(() => {
  ...
  desktopCode.shouldUseLegacyProtectedCodeAttachmentFallback.mockImplementation(
    (error: unknown) =>
      !tauri.enabled &&
      error instanceof api.CantripApiError &&
      error.status === 409 &&
      error.code === "shared-code-transport-requires-single-server",
  );
