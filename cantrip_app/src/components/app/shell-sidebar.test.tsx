...
const mocks = vi.hoisted(() => ({
  ...
  settingsButtonProps: null as Record<string, any> | null,
}));

vi.mock("@/components/ui/tooltip", () => ({
  TooltipButton: ({ tooltip, tooltipSide, ...props }: Record<string, any>) => {
    mocks.settingsButtonProps = { ...props, tooltip, tooltipSide };
    return <button {...props} />;
  },
}));
...
describe("desktop app mode menu", () => {
  ...
  it("keeps the file tree mounted through pinning and successor creation", () => {
    ...
    bindings.showProjectSettings = true;
    mocks.projectChatListProps.length = 0;
  ...
  it.each([false, true])(
    "toggles settings from the sidebar when open=%s",
    (showSettings) => {
      ...
      const button = mocks.settingsButtonProps!;
      expect(button.tooltip).toBe(label);
      expect(button.tooltipSide).toBe("top");
      expect(button["aria-pressed"]).toBe(showSettings);
      expect(button.className.includes("bg-muted")).toBe(showSettings);
      button.onClick();
      expect(setShowSettings).toHaveBeenCalledWith(!showSettings);
      if (showSettings) expect(setSettingsSection).not.toHaveBeenCalled();
      else expect(setSettingsSection).toHaveBeenCalledWith("general");
    },
  );
});
