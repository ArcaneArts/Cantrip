...
export function createCuaMcpServer(gateway: CuaMcpGateway) {
  ...
  server.registerTool(
    ...
    {
      description:
        "Use cua to inspect application windows, capture screenshots, discover pressable controls with cua.controls(), and perform a real action with cua.press(reference) or a single left click with cua.click({x,y}). Native input requires mutation authorization; moveCursor stays logical-only.",
