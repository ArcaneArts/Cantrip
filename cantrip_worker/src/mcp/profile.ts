...
export function cantripMcpInstructions(profile: CantripMcpProfile): string {
  return profile === "standalone-web"
    ...
    : "Use Cantrip MCP only for Cantrip-owned state, surfaces, and worker-managed web research. Call context_get first; its contextWindow field reports the latest exact Codex context occupancy when available. Call context_compact when the user requests /compact or when the reported remaining window is inadequate for imminent high-volume work; native compaction runs at the safe idle boundary after the current turn, so end the turn immediately after it succeeds. Call tool_help with a tool name before guessing arguments; it returns exact schema generated from the live authoritative validator. Use web_search for bounded discovery and web_read with its opaque result IDs or continuation cursors for static page content. Use web_session_open only when interaction is required, take a fresh web_session_snapshot when the page changes and never reuse stale element references. For an exact open Browser target, web_session_pointer supports viewport CSS-pixel movement, clicking, and dragging; user interaction does not cancel the agent session. Read effective policies when a summary requires the full body. List authorized targets; never guess or reuse IDs. Use run_configuration_detect to discover typed targets and run_configuration_list or run_configuration_get to obtain stable configuration IDs and exact revisions. Create and update structured definitions with explicit operation IDs; never select a configuration or worktree by display name. A Run targets Primary unless an exact worktree ID is supplied. Use explicit start, restart, stop, status, and read-output operations for one configuration/worktree runtime identity. Secret values are write-only through run_configuration_secret_set. Use the worker-authenticated Cantrip CLI as the fallback. End the turn immediately if continuationScheduled is true. Treat the binding scope as authoritative. Do not retry denied, expired, or stale calls without refreshed context.";
}
