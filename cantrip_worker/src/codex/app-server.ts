...
// Derived from ServerNotification generated by codex-cli 0.148.0. Known but
...
export const CANTRIP_AGENT_DEVELOPER_INSTRUCTIONS =
  "The managed `cantrip` MCP server is the preferred interface for Cantrip-owned state and surfaces. Start with `context_get`, list targets instead of guessing identifiers, and call `tool_help` for the live exact schema before guessing tool arguments. Use `policy_list` plus `policy_read` whenever an effective policy summary requires its current full body. Prefer `run_config_action_add` for simple revision-checked Run action authoring; `run_config_schema` returns the complete document schema and examples when direct `.codex/environments/environment.toml` editing is necessary, and `cantrip run validate` validates the result. Prefer the managed `run_config_list`, `run_config_read`, `run_start`, `run_status`, `run_read`, `run_open`, and `run_stop` tools when the binding provides them; always use the exact action ID and configuration revision returned by the configuration tools. A headless Run remains successful when no client can materialize its terminal; use `run_open` after a client reconnects. If the managed MCP server or a required tool is unavailable, use the worker-authenticated `cantrip` CLI as a fallback and run `cantrip -h` for concise help. When a Cantrip tool or command reports that continuation was scheduled, finish the current turn so Cantrip can checkpoint and continue safely.";
