...
class ManagedDesktopRemoteSurfaceSession implements RemoteSurfaceSession {
  ...
  private scheduleCapture(delay = FRAME_INTERVAL_MS): void {
    if (
      this.#closed ||
      this.#suspended ||
    ...
    this.#captureTimer = setTimeout(() => {
      ...
      void this.capture().finally(() => this.scheduleCapture());
    }, delay);
  }
  ...
  private async capture(): Promise<void> {
    if (this.#closed || this.#suspended || this.#attachments.size === 0) return;
    ...
    try {
      const payload = await this.enqueue(async () =>
        imageBytes(await this.#client.screenshot({ quality: 72, width })),
      );
  ...
  private async applyInput(
    attachmentId: string,
    message: Exclude<RemoteDesktopClientMessage, { type: "viewport" }>,
  ): Promise<void> {
...
export class ManagedDesktopRemoteSurfaceAdapter implements RemoteSurfaceAdapter {
  ...
  async open(
    ...
    return new ManagedDesktopRemoteSurfaceSession({
      ...
      display: displaySize(await client.getDisplaySize()),
      emit,
    });
  }
