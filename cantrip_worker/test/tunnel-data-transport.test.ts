...
describe("worker generic tunnel data transport", () => {
  ...
  it.each([
    { managed: false, ok: true },
    { managed: true, ok: true },
    { managed: true, ok: false },
  ])(
    "delivers the original logical root with a chat outcome after reconnect (managed=$managed, ok=$ok)",
    async ({ managed, ok }) => {
      const server = createServer();
      ...
      const connection = new WorkerConnection(
        ...
        async () => {
          commandStarted?.();
          await gate;
          if (!ok) throw new Error("Fixture turn failure");
          return {
            threadId: "thread-1",
            turnId: "turn-1",
      ...
      const logicalRoot = {
        operationId: "original-root",
        operationGeneration: "original-generation",
      };
      const command = {
        ...chatTurnCommand(),
        ...(managed
          ? {
              nativeCommandReceipt: nativeCommandReceiptSchema.parse({
                ...logicalRoot,
      ...
      expect(envelopes).toContainEqual({
        ...
        notification: {
          ...
          scratchRootId: null,
          worktreeId: "worktree-1",
          ...(managed ? { nativeLogicalRoot: logicalRoot } : {}),
          outcome: ok
            ? {
                ok: true,
                result: {
            ...
            : { ok: false, error: "Fixture turn failure" },
        },
      });
    },
  );
