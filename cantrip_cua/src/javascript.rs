...
((host, stringify) => {
  ...
  Object.defineProperty(globalThis, 'cua', { value: Object.freeze({
    ...
    keyChord: (keys, holdMs = 500, modifiers = []) => {
      if (!Array.isArray(keys) || !Array.isArray(modifiers)) throw new Error('keyChord expects key and modifier arrays');
      const names = ['Meta','Control','Alt','Shift'];
      const flags = [...new Set([...modifiers, ...keys.filter(key => names.includes(key))])];
      const held = keys.filter(key => !names.includes(key));
      if (!held.length) throw new Error('keyChord requires a non-modifier key');
      return call({operation:'perform', command:{kind:'timeline',frames:[{atMs:0,keyDown:held,...(flags.length ? {keyModifiers:flags} : {})},{atMs:holdMs,keyUp:held}]}});
    },
    ...
    click: (point, options = {}) => {
      if (options === null || typeof options !== 'object' || Array.isArray(options) ||
          Object.keys(options).some(key => !['button','modifiers','holdMs'].includes(key)))
        throw new Error('click options are {button,modifiers,holdMs}');
      return pointerPress(point, options.holdMs ?? 150, options.modifiers ?? [], options.button ?? 'left');
    },
...
mod tests {
    ...
    fn macro_bootstrap_sends_bounded_host_commands() {
        for (script, expected) in [
            (
                r#"await cua.keyPress("k",["Meta"])"#,
                json!({"operation":"perform","command":{"kind":"key","key":"k","modifiers":["Meta"]}}),
            ),
            (
                r#"await cua.keyChord(["Meta","k"])"#,
                json!({"operation":"perform","command":{"kind":"timeline","frames":[{"atMs":0,"keyDown":["k"],"keyModifiers":["Meta"]},{"atMs":500,"keyUp":["k"]}]}}),
            ),
            (
                r#"await cua.keyChord(["k"],500,["Meta"])"#,
                json!({"operation":"perform","command":{"kind":"timeline","frames":[{"atMs":0,"keyDown":["k"],"keyModifiers":["Meta"]},{"atMs":500,"keyUp":["k"]}]}}),
            ),
            (
                r#"await cua.click({x:12,y:34},{button:"back"})"#,
                json!({"operation":"perform","command":{"kind":"prepared-press","point":{"x":12,"y":34},"holdMs":150,"button":"back"}}),
            ),
            (
                r#"await cua.pointerPress({x:12,y:34},100,["Control"],"middle")"#,
                json!({"operation":"perform","command":{"kind":"timeline","frames":[{"atMs":0,"pointerDown":{"x":12,"y":34},"pointerModifiers":["Control"],"pointerButton":"middle"},{"atMs":100,"pointerUp":true}]}}),
            ),
            (
                r#"await cua.keyChord(["A"],200,["Meta","Shift"])"#,
                json!({"operation":"perform","command":{"kind":"timeline","frames":[{"atMs":0,"keyDown":["A"],"keyModifiers":["Meta","Shift"]},{"atMs":200,"keyUp":["A"]}]}}),
            ),
            (
                r#"await cua.mediaKey("VolumeUp",["Shift"])"#,
                json!({"operation":"perform","command":{"kind":"media","key":"VolumeUp","modifiers":["Shift"]}}),
            ),
            (
                r#"await cua.click({x:12,y:34})"#,
                json!({"operation":"perform","command":{"kind":"prepared-press","point":{"x":12,"y":34},"holdMs":150}}),
            ),
            (
                r#"await cua.click()"#,
                json!({"operation":"perform","command":{"kind":"prepared-press","holdMs":150}}),
            ),
            (
                r#"await cua.pointerPress({x:12,y:34},7200000,[])"#,
                json!({"operation":"perform","command":{"kind":"prepared-press","point":{"x":12,"y":34},"holdMs":7200000}}),
            ),
            (
                r#"await cua.preparedPointerPress({x:12,y:34},1000)"#,
                json!({"operation":"perform","command":{"kind":"prepared-press","point":{"x":12,"y":34},"holdMs":1000}}),
            ),
            (
            ...
            (
                ...
                json!({"operation":"perform","command":{"kind":"prepared-press","point":{"x":10,"y":20},"holdMs":200}}),
            ),
            (
            ...
            (
                r#"await cua.clickDrag({x:1,y:2},{x:3,y:4},150000)"#,
                json!({"operation":"perform","command":{"kind":"drag","start":{"x":1,"y":2},"end":{"x":3,"y":4},"durationMs":150000}}),
            ),
            (
                r#"await cua.clickDrag({x:1,y:2},{x:3,y:4},0)"#,
                json!({"operation":"perform","command":{"kind":"drag","start":{"x":1,"y":2},"end":{"x":3,"y":4},"durationMs":0}}),
            ),
            (
                r#"await cua.clickDrag({x:1,y:2},{x:3,y:4},Number.MAX_SAFE_INTEGER)"#,
                json!({"operation":"perform","command":{"kind":"drag","start":{"x":1,"y":2},"end":{"x":3,"y":4},"durationMs":9_007_199_254_740_991_u64}}),
            ),
            (
    ...
    fn host_actions_only_accept_implemented_operations_and_no_authority_fields() {
        assert!(
            validate_action(r#"{"operation":"capture.request","request":{"type":"open"}}"#)
                .is_err()
        );
        assert!(
