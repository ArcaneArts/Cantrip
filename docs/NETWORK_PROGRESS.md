...
T2.7B, and the Remote Desktop cutover is complete through T2.8. T2.9A now
supplies exact observation authority,
replica-safe grant issuance, the shared event-subscription channel, and worker
fan-out without changing the canonical server path. T2.9B must connect the
supported client, provisional chat/runtime caches, canonical deduplication, and
authoritative resync. Relay consolidation and the final acceptance gate then
follow. TURN and transparent TCP resumption remain explicitly deferred, and the
restored Code/Explorer lifecycle remains a mandatory regression boundary.
