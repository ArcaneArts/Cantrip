...
| 6B — Shared desktop Code pool   | Move the process-wide Tauri Code transport pool and handoff behind WorkerLink              | In progress | `codex/network-tranche1-pass6b-code-pool`      | Pending                                                  | Workspace/app typecheck; focused app 70 and full app 1,572; Rust fmt/check and WorkerLink/pool 4; source/CLI verification; changed-file format and diff checks | Preserves generation-fenced multi-window leases, carrier handoff, and stable localhost ownership without feature fallback   |
