...
This is the execution ledger for the network fabric. Tranche One built the
shared WorkerLink session, authorization, routing, and reliable-stream boundary
while preserving `LOCAL -> RELAY`. Tranche Two activated direct LAN/WAN
carriers, mobility, shared feature adapters, relay consolidation, and the final
stabilization gate.
...
Tranche Two activated the direct-route system in independently mergeable
passes: bounded peer contracts and policy, authenticated signaling, WebRTC and
native bridge carriers, route selection and mobility, feature migrations,
direct observations, shared relay hardening, compatibility isolation, and a
repeatable acceptance matrix. TURN and transparent TCP resumption remain
...
| T2.7B — Browser Remote Surface client            | Cut the supported Browser UI over to shared WorkerLink lanes, bounded frame fragmentation, reconnect, and route truth                        | Complete | `codex/network-tranche2-pass7b-browser-client`           | [#1206](https://github.com/ArcaneArts/Cantrip/pull/1206) | Focused protocol 16, worker 5, app 20, and server placement API 14; full protocol 365; full app 1,606 pass and 3 skip; full worker 897 pass, 2 skip, and 2 unrelated failures: the repeatable direct-broker logging race reproduces on unchanged `main`, while the intermittent goal-streaming case passes in isolation; workspace typecheck; app build; topology and Code/Codex source verification; formatting/diff checks | Browser development-service tunnels retain the generic `stream` adapter; the legacy Remote Surface transport remains for Remote Desktop and compatibility clients until later passes; fresh-worktree app build required building the local `@cantrip/glitch` dependency first                                                                                                                                                     |
| T2.8 — Remote Desktop migration                  | Move Remote Desktop input, clipboard, lifecycle, frames, reconnect, congestion, and route truth behind shared WorkerLink lanes               | Complete | `codex/network-tranche2-pass8-remote-desktop`            | [#1207](https://github.com/ArcaneArts/Cantrip/pull/1207) | Focused server 15, worker adapter/desktop 9, and shared app client/UI 11; full app 1,608 pass and 3 skip; full worker 899 pass and 2 skip with one order-dependent Code-settings failure that passes alone; workspace typecheck; app build; topology and Code/Codex source verification; formatting/diff checks                                                                                                              | Reuses the T2.7 bounded Remote Surface substrate; the feature-specific Remote Surface transport remains only for compatibility clients until T2.10; the broad server suite retains unrelated baseline failures/timeouts, including a Z.ai catalog constraint failure reproduced unchanged on `main`                                                                                                                               |
...
| T2.11 — Full acceptance and stabilization gate   | Execute and preserve the complete deterministic topology, failure, feature, authority, replica, native bridge, and Code/Explorer matrix      | Complete | `codex/network-tranche2-pass11-acceptance`               | [#1212](https://github.com/ArcaneArts/Cantrip/pull/1212) | Deterministic protocol 18, app 178, worker 42, server 35, and native Tauri 5; full protocol 369; full app 1,614 pass and 3 skip; full worker 904 pass and 2 skip; workspace typecheck/build; Android/iOS Capacitor sync; native format/check; Code/Codex source and CLI verification; acceptance/source/format/diff checks                                                                                                   | The broad worker suite exposed a non-aligned TCP credit-boundary deadlock; the adapter now splits queued reads at available credit and the exact regression is part of the deterministic gate. A recurring DirectBroker test race now waits for the server-side disconnect log after client close. Physical iOS, Android, multi-device LAN, cellular, and configurable NAT checks were unavailable and remain explicitly Not run. |

## Post-stabilization repairs

| Repair                             | Scope                                                                                                                           | Status   | Branch                                | PR                                                       | Validation                                                                                                                           | Notes or deviations                                                                                                                                                                                                                                               |
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | -------- | ------------------------------------- | -------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| R1 — Same-machine LOCAL continuity | Activate and renew exact LOCAL capabilities, promote reconnectable observation streams, and preserve truthful route diagnostics | Complete | `codex/fix-worker-link-local-renewal` | [#1213](https://github.com/ArcaneArts/Cantrip/pull/1213) | Deterministic protocol 19, app 180, worker 42, server 80, and native Tauri 5; package typechecks; topology/source/format/diff checks | LOCAL heartbeats previously renewed a capability the server still considered inactive, so the capability expired and the link settled on RELAY. Reliable feature streams remain pinned; provisional event subscriptions reconnect on a promoted route and resync. |

## Tranche Two stabilization evidence

| Acceptance surface        | Evidence                                                                                                                                                                                                                           | Result                         |
| ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------ |
| Four-route selection      | Named deterministic cases prove `LOCAL -> LAN -> WAN -> RELAY`, concurrent RELAY readiness, LAN/WAN promotion, and direct-failure fallback                                                                                         | Pass                           |
| Mobility and mixed routes | Cellular/Wi-Fi failure injection reprobes in priority order; reliable streams retain their route while reconnectable event subscriptions and new channels promote; per-channel failure does not collapse a healthy sibling carrier | Pass                           |
| Peer authentication       | DTLS WebRTC handshakes bind the exact server, account session, client instance, worker process, peer session, and route generation; stale or spoofed identity fails closed                                                         | Pass                           |
| Feature neutrality        | Terminal, Code, generic tunnels, Browser, Remote Desktop, and observations enter shared WorkerLink adapters; both topology audits reject feature-owned route selection                                                             | Pass                           |
| Code/Explorer boundary    | Preview, pinned ownership, attachment-ready, transient reconnect, service-worker, and no-speculative-spare tests pass without adding sessions, iframes, or prewarming                                                              | Pass                           |
| Authorization and scale   | Expiry, logout, resource deletion, worker replacement, server generation, multiple clients, multiple workers, and replica-separated control paths retain exact fencing                                                             | Pass                           |
| Relay independence        | Relay-only policy, bounded QoS queues, feature quotas, metering, backpressure, cross-replica delivery, and revocation remain independently executable                                                                              | Pass                           |
| Platform boundary         | Shared browser/Capacitor renderer PeerCarrier and native Tauri bridge pass; Android/iOS projects sync; unavailable physical-device checks are disclosed rather than inferred                                                       | Pass with physical QA deferred |

## Tranche Two acceptance checklist

- [x] LAN and WAN are operational WorkerLink routes.
- [x] Route priority is `LOCAL -> LAN -> WAN -> RELAY` and RELAY is prepared concurrently.
- [x] Browser and Capacitor share direct WebRTC for supported in-app features.
- [x] Tauri tunnels and Code preserve stable localhost listeners across LAN/WAN reconnects.
- [x] Browser and Remote Desktop share WorkerLink interactive/realtime lanes.
- [x] Provisional chat and worker observations can bypass relay while final state remains server-authoritative.
- [x] Network changes reprobe and observation/stream controllers resubscribe safely.
- [x] Per-channel mixed routes and fallback are supported.
- [x] Unauthorized, expired, replayed, and stale-generation peer traffic fails closed.
- [x] Multiple coordinated server replicas preserve routing and revocation semantics without sticky sessions.
- [x] Relay-only operation remains forceable and fully covered.
- [x] Supported feature components contain no topology-selection branch.
- [x] Code/Explorer preview, pinned-tab, readiness, and reconnect behavior remains stable.
- [x] The deterministic matrix is repeatable and physical-device gaps are recorded honestly.
- [x] Architecture, progress, acceptance evidence, deviations, and deferred decisions are synchronized.

## Tranche One stabilization acceptance evidence
...
- T2.11 validates the shared browser/Capacitor renderer path deterministically
  ...
  checks are explicitly `Not run` in `NETWORK_ACCEPTANCE.md`; they are not
  inferred from renderer WebRTC tests.
- Browser/WebView Network Information events remain implementation-dependent.
  ...
  explicit reprobe, and active ICE/carrier failure. A browser or Tauri WebView
  that emits no environment change may retain a healthy lower-priority route
  until an authorized upgrade probe or carrier event; correctness and fallback
  are unaffected.
...
- The T2.4A full worker run reports 892 passing tests, two skipped, and one
  ...
  T2.11 removed this assertion race by waiting for the server-side disconnect
  record after the client close event; the current full worker suite passes.
...
- Remote Desktop uses the same exact attachment grant and shared Remote Surface
  client, with pointer, keyboard, clipboard, inventory, and lifecycle traffic
  on `interactive` and disposable desktop/cursor frames on `realtime`. The
  worker registers the same lane adapter for `remote-desktop`; a first ready
  viewport republishes current state and encrypted target inventory after the
  lanes exist, and the UI exposes truthful same-route or mixed per-lane route
  labels without selecting WebRTC, TURN, or relay URLs.
...
- The client WorkerLink manager now owns a subscribable current-client route
  ...
  private addresses, or peer candidates. T2.4 activated LAN and WAN without
...
- The common server `WorkerLinkRelay` now applies the existing account and
  worker relay-byte limits, records per-feature ingress and egress, shares one
  Remote Surface connection quota across the grant's interactive and realtime
  lanes, and bounds each QoS lane independently. Congested bulk or realtime
  traffic cannot consume the interactive queue, a stalled client fails closed,
  and operations diagnostics expose aggregate queue pressure without tenant or
...
and stable localhost listeners. Browser Remote Surface is complete through
T2.7B, and the Remote Desktop cutover is complete through T2.8. T2.9A supplies
exact observation authority, replica-safe grant issuance, the shared
event-subscription channel, and worker fan-out. T2.9B connects that substrate to
...
soak without prematurely removing distributed-client server endpoints. T2.11
closes the tranche with a repeatable named matrix and stabilized architecture,
progress, and acceptance records. No Tranche Two implementation pass remains.
TURN and transparent TCP resumption stay explicitly deferred, and the restored
Code/Explorer lifecycle remains a permanent regression boundary.

Post-stabilization repair R1 closes a same-machine continuity gap found after
the gate: the server now explicitly activates the exact LOCAL capability before
accepting lease heartbeats, repeated renewals preserve that route, and renewal
failure enters the ordinary priority-ordered recovery path instead of being
silently ignored. A better route reconnects provisional event subscriptions and
performs authoritative resync without disrupting arbitrary reliable streams.
Session renewal no longer overwrites the route transition reason, so the
Network Map reports why the effective route actually changed.
