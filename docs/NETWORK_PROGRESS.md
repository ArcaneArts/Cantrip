...
ADR 0010 selects the bounded WebView bridge for native Tauri traffic. T2.6B
hardening and parity implementation is next, followed by Browser Remote Surface,
Remote Desktop, worker observations, incremental chat, filesystem watcher
migration, relay consolidation, and the final acceptance gate. Native desktop
tunnels still map LAN/WAN stream labels onto their existing direct summary
state until T2.6B completes the physical native carrier. TURN and transparent
