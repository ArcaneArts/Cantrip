...
| S7 — Code preview prewarm     | Keep the next sidebar Code preview ready while the current preview is active                 | Complete | `codex/fix-new-code-tab-prewarm`         | [#1194](https://github.com/ArcaneArts/Cantrip/pull/1194) | App typecheck/build; focused Explorer and prewarm 30; full app 1,585 with 3 skipped; Tranche One topology verification; changed-file formatting and diff checks                                  | LOCAL tunnel health, HTTP, and WebSocket setup were already completing in milliseconds; the remaining new-tab delay came from cold embedded workbench startup after the only warm preview was pinned            |
