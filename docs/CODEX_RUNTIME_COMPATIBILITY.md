...
Cantrip currently pins and builds `codex-cli 0.150.1` from the official
`rust-v0.150.1` source tag. Its resolved commit, imported source manifest, and
...
The adapter's compatibility range is `>=0.150.0 <0.151.0`, but packaged
workers contain exactly `0.150.1`; they do not select another compatible patch
...
For 0.150 it also probes process diagnostics, all six durable thread-queue
...
Portable ChatGPT accounts depend on an experimental Codex 0.150 App Server
surface. Before starting a server-managed ChatGPT runtime, the worker requires:

- semantic version `0.150.x`;
...
This integration required no patch to the imported Codex 0.150 source. It is
...
Product readiness may be stricter than method discovery. Codex 0.150 stabilizes
