...
| MCP names, commands, URLs, headers, environment, and configuration                                                          | Complete row-bound AES-256-GCM configuration under `mcp-secret`; keyed name blind index; worker binding is routing metadata                                                                           | E2EE complete                                    | Implemented          | High        | Server can route by global/project/worker scope and blind override key but cannot validate or describe configuration                                                        |
