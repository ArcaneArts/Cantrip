...
| Repository identities and names, remotes, paths, branch names, and Git output    | `repository-content` operations; keyed identity blind indexes; worker-local opaque routing for identity, setup, lifecycle, paths, branches, status, and operation state   | E2EE complete except Git-agent workflow drafts   | Partial              | High                                                       | Repository/Git content, identity, paths, branches, status, errors, and GitHub catalog data; equality leakage remains |
...
The remaining exception for the combined ledger row is Git-agent draft
generation. Its prompts and model output share the workflow runtime and will be
closed with workflow content rather than introducing a second repository-only
model relay. Commit hashes and repository fingerprints remain intentionally
plaintext revision identifiers: they route and compare immutable revisions and
are not treated as user-authored content.
...
18. **Protected repository operations — primary path complete:** stateless
    ...
    the server.
19. **Repository metadata and legacy-route closure — complete except Git-agent
    workflow drafts:** keyed blind indexes and worker routing handles protect
    ...
    their legacy plaintext routes fail closed. Git-agent prompts/results move
    with workflow content before the combined repository ledger row earns
    `E2EE complete`.
