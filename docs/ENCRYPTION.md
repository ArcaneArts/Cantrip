...
| MCP names, commands, URLs, headers, environment, and configuration                                                          | Row-bound AES-256-GCM under `mcp-secret`; keyed name blind index; worker-local discoveries are encrypted before relay; worker binding is routing metadata                                             | E2EE complete                                    | Implemented          | High        | Server can route by scope and blind override key but cannot inspect configured or discovered plaintext                                                                    |
...
files on the selected worker. Importable entries are assigned a prospective row
ID and encrypted by the worker under `mcp-secret` before the discovery result is
relayed. The server receives only the row-bound encrypted create payload and
...
partially protected user-payload rows in this ledger.

The database-compromise guarantee is therefore closed for the stated target:
someone who obtains server database contents but not the user's login password,
an authorized client device private key, or an authorized worker private key
cannot decrypt user payloads. Server-readable exceptions are limited to the
explicitly classified routing/control/public state, hashed validators, opaque
relationships and traffic shape, and minimized numeric/operational records.
Authorized worker-local plaintext remains an endpoint property, not server
custody. No recovery secret, local encryption password, or second user-managed
credential exists.
