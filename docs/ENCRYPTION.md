...
- Workflow definition slugs, names, descriptions, and provenance plus revision
  ...
  client-encrypted under `workflow-content`. Run inputs/results, attempts,
  interactions, triggers, deliveries, and private errors still require the
...
| Workflow revision graphs, node names/prompts/configuration, edge predicates, schemas, defaults, and permission requirements   | One row-bound opaque revision definition plus a minimized public scheduling manifest of random IDs, topology, primitive type, read/write mode, and routing IDs                            | E2EE complete; execution temporarily fail-closed | Implemented          | High                                                       | Server cannot inspect or validate authoring semantics; it retains only the topology and classifications needed by the scheduler   |
| Workflow run inputs/results, node/item state, attempts, interactions, gates, events, triggers, deliveries, and private errors | Plaintext compatibility storage; new execution and trigger ingress fail closed while protected worker execution is implemented                                                            | Core workflow runtime E2EE planned               | Good when split      | Very high                                                  | Scheduler can route opaque jobs, but content evaluation and result validation must move to workers                                |
...
Manual run creation and every trigger mutation/delivery route now return `410`.
This prevents the legacy server executor from receiving or synthesizing
plaintext definition content while the protected worker-runtime protocol is
built. Read-only run/trigger endpoints remain for empty-state and historical
control-plane compatibility after the workflow-only reset.

Run inputs and outputs, node and item state, attempts, workflow-node
interactions, events, gate prompts/reasons, trigger configuration/input,
delivery payloads, and private errors are still plaintext compatibility fields.
...
21. **Core workflow definitions — complete; runtime ongoing:** definition
    ...
    generation/repository/save paths and all run/trigger ingress fail closed.
    Execution payloads and worker-side interpretation remain the next phases.
