...
URLs, and Remote Desktop target/inventory details use the independently scoped
`surface-private-state` component described below. Code runtime state,
Explorer operation paths, browser page content, Remote Desktop frames/input/
clipboard, placement, and public routing status remain separate ledger work.
...
interactive input, clipboard contents, and binary relay encryption remain a
separate planned ledger row and are not claimed by this milestone.
...
7. **Task content — complete:** the Task vertical slice encrypts core rows,
   ...
   and workflow interaction payloads remain planned; ordinary chat approvals
   now use the endpoint-only `interaction-content` path. Task/chat titles are
   tracked separately and are now E2EE complete.
...
10. **Ordinary interactions — primary path complete:** workers encrypt
...
11. **Attachments and relayed streams:** encrypt metadata and add
    application-layer encryption when bytes traverse relays.
12. **Secrets:** replace server-decryptable provider and MCP vault envelopes
    with client and worker decryptable envelopes.
13. **Private surface metadata persistence — complete:** Remote Desktop
...
14. **Workflows and optional private analytics:** split scheduling metadata
