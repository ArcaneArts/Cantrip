...
- Project automation names, prompts, and conditions are client-encrypted under
  ...
  Core workflow definitions, inputs, and results are not yet encrypted.
...
| Workflow definitions, node prompts, structured inputs, and results               | `workflow-content` codec and project-automation adapter complete; core workflow rows and execution remain plaintext                                                                       | Core workflow E2EE planned                       | Good when split      | Very high                                                  | Scheduler can route opaque jobs, but content evaluation and result validation must move to workers                                |
...
Core workflow definitions, revision graphs, run inputs and outputs, node
configuration, attempts, interaction payloads, events, trigger payloads,
delivery payloads, and private errors are still plaintext. Closing them requires
moving semantic validation, predicate evaluation, node prompt composition, and
result interpretation to authorized workers while retaining only scheduler
state, dependencies, leases, retries, deadlines, and opaque relationships on
the server.
...
21. **Core workflows and optional private analytics:** split scheduling metadata
