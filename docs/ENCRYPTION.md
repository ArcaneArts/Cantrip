...
- Workflow definition slugs, names, descriptions, and provenance plus revision
  provenance and content hashes are client-encrypted under `workflow-content`.
  Revision node/edge content, run inputs/results, attempts, interactions,
  triggers, deliveries, and private errors still require the worker-runtime
  cutover.
...
| Workflow node/edge content, schemas, defaults, run inputs/results, attempts, interactions, gates, triggers, deliveries, and private errors | Plaintext compatibility path while semantic execution moves to authorized workers                                                                                                         | Core workflow runtime E2EE planned               | Good when split      | Very high                                                  | Scheduler can route opaque jobs, but content evaluation and result validation must move to workers                                |
...
operations. This is the shared cryptographic foundation for the remaining core
workflow rollout; adding the codec does not by itself make those server rows
opaque.
...
revision reads reconstruct the still-compatible graph from node and edge rows.
This closes an unnecessary duplicate plaintext copy without pretending the
underlying node and edge content is already encrypted. Server-side chat
generation, repository scan/import/export, and save-run-as-revision routes now
return `410` until their protected client/worker relays replace the plaintext
implementations.
...
metadata round trips, row binding, opaque persistence/relay, removal of legacy
columns and the duplicate graph, and fail-closed plaintext producers.

Revision node keys/names/configuration/prompts, edge predicates, declared
schemas/defaults/permissions, run inputs and outputs, node and item state,
attempts, workflow-node interactions, events, gate prompts/reasons, trigger
configuration/input, delivery payloads, and private errors are still plaintext.
Closing them requires moving semantic validation, predicate evaluation, node
prompt composition, and result interpretation to authorized workers while
retaining only scheduler state, dependencies, leases, retries, deadlines, and
opaque relationships on the server.
...
21. **Core workflow catalog — complete; runtime ongoing:** definition slugs,
    names, descriptions, and provenance plus revision provenance and content
    hashes are client-encrypted, blind-indexed where equality is operationally
    required, and never interpreted by the server. The duplicate aggregate
    graph is removed and legacy plaintext generation/repository/save paths fail
    closed. Node/edge content and execution payloads remain the next worker-side
    phases.
