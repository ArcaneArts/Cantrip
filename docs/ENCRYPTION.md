...
| Repository identities and names, remotes, paths, branch names, and Git output    | Git, History, Issues, pull-request, release, and managed-operation content use `repository-content`; managed-operation state is worker-local; identity/path/worktree state plus Git-agent and checkout routes remain plaintext | Protected operations complete; metadata closure pending | Partial              | High                                                       | Server cannot inspect command arguments, Git output, managed-operation context, issue/PR bodies, or errors |
...
row. Git-agent drafts, pull-request checkout, worktree status/lifecycle calls,
repository identity, source and worktree paths, branch/HEAD state, and the
remaining durable metadata are reserved for the next repository-metadata
cutover.
...
18. **Protected repository operations — primary path complete:** stateless
    ...
    required by the legacy repository model. Managed-operation context and
    output are durable on the authorized worker instead of in the server
    database, and migrated plaintext content routes fail closed. A static
    dependency and route audit keeps trusted content schemas and crypto outside
    the server.
19. **Durable repository metadata and legacy-route closure:** encrypt
    ...
    Git-agent drafts, checkout preparation, and remaining private status/error
    fields; move canonical execution context to authorized endpoints; then
    remove or fail closed every remaining plaintext content route before the
    combined repository ledger row earns `E2EE complete`.
