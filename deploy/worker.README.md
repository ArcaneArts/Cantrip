...
CLI built and verified with this worker release. Use the same worker token
configured on the server. The worker makes the outbound server connection;
apps never connect to workers directly.
