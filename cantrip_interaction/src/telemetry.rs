//! Generated input/presentation telemetry. Never evidence of application acceptance.
use crate::{
    cursor::CursorState,
    geometry::{Bounds, Point},
    presentation::{CursorSource, CursorTarget},
};
pub const MAX_RENDERED_PARTICIPANTS: usize = 16;
...
const EVENTS_PER_PARTICIPANT: usize = 32;
...
impl Participant {
    fn new<S: CursorSource>(state: &S, target: &S::Target, now: u64) -> Self {
        let identity = state.appearance_identity();
    ...
    pub fn input(&mut self, event: InputEvent, now: u64) {
        ...
        while self.events.len() > EVENTS_PER_PARTICIPANT {
...
impl Telemetry {
    pub fn synchronize(&mut self, states: &[impl CursorSource], now: u64) {
        self.participants.retain(|id, _| {
            states
                .iter()
                .any(|s| s.participant_id() == id && s.target().is_some())
        });
        for state in states {
            ...
            let participant = self
                .participants
                .entry(state.participant_id().to_owned())
                .or_insert_with(|| Participant::new(state, target, now));
            if participant.target_id != target.id() || participant.generation != target.generation()
            {
                *participant = Participant::new(state, target, now);
            }
            let resized = participant.bounds.width != target.bounds().width
                || participant.bounds.height != target.bounds().height
                || participant.scale != target.scale_factor();
            if state.cursor().revision != participant.revision || resized {
                participant.move_to(state.cursor().position, now, resized);
                participant.revision = state.cursor().revision;
            }
            participant.appearance(state.cursor());
            participant.bounds = target.bounds();
            participant.scale = target.scale_factor();
        }
    }
    pub fn geometry(&mut self, target: &impl CursorTarget, now: u64) {
        for participant in self
            .participants
            .values_mut()
            .filter(|a| a.target_id == target.id() && a.generation == target.generation())
        {
            if participant.bounds.width != target.bounds().width
                || participant.bounds.height != target.bounds().height
                || participant.scale != target.scale_factor()
            {
                participant.move_to(participant.position, now, true);
            }
            participant.bounds = target.bounds();
            participant.scale = target.scale_factor();
        }
    ...
    pub fn movement(
        &mut self,
        session: &str,
        target: &impl CursorTarget,
        point: Point,
        now: u64,
        discontinuity: bool,
    ) {
        if let Some(participant) = self.matching(session, target) {
            participant.move_to(point, now, discontinuity);
        }
    }
    pub fn input(
        &mut self,
        ...
        if let Some(participant) = self.matching(session, target) {
            participant.input(event, now);
        }
    }
    fn matching(&mut self, session: &str, target: &impl CursorTarget) -> Option<&mut Participant> {
        self.participants
            .get_mut(session)
            .filter(|a| a.target_id == target.id() && a.generation == target.generation())
    }
    pub fn window(&self, id: &str, generation: u64, now: u64) -> Vec<Participant> {
    ...
    pub fn window_with_dissipation(
        ...
        result.truncate(MAX_RENDERED_PARTICIPANTS);
        result
    }
}
